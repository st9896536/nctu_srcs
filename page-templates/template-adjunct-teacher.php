<?php
/*
 * Template Name: adjunct-teacher
 */
?>


<?php $thisPage="adjunct-teacher"; ?>

<head>
  <link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/css/page.css" type="text/css" />

</head>

<?php get_template_part('includes/header'); ?>
</?php get_template_part('includes/sidebar'); ?>

<div class="sidebarmenu">

  </?php $URL=$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
  echo $URL;
  ?>
  <a <?php if ($thisPage=="aboutus") echo " id=\"currentpage\""; ?> href="<?php echo site_url(); ?>/aboutus" class="sidebartext">關於本所</a>
  <a <?php if ($thisPage=="full-time-teacher") echo " id=\"currentpage\""; ?> href="<?php echo site_url(); ?>/full-time-teacher" class="sidebartext">專任師資</a>
  <a <?php if ($thisPage=="adjunct-teacher") echo " id=\"currentpage\""; ?> href="<?php echo site_url(); ?>/adjunct-teacher" class="sidebartext">兼任師資</a>
  <a <?php if ($thisPage=="guest-professor") echo " id=\"currentpage\""; ?> href="<?php echo site_url(); ?>/guest-professor" class="sidebartext">客座教授</a>
  <a <?php if ($thisPage=="administration-staff") echo " id=\"currentpage\""; ?> href="<?php echo site_url(); ?>/administration-staff" class="sidebartext">行政人員</a>
  <a <?php if ($thisPage=="current-student") echo " id=\"currentpage\""; ?> href="<?php echo site_url(); ?>/current-student" class="sidebartext">在校生</a>
  <a <?php if ($thisPage=="graduate") echo " id=\"currentpage\""; ?> href="<?php echo site_url(); ?>/graduate" class="sidebartext">畢業生</a>
</div>


<div class="container">
  <div class="row">
    <div class="main" >
      <div class="main_long"><font>兼任師資</font></div>
      <div class="name_block" style="width: 71.875vw; display: inline-flex; margin-top: 4vh; margin-bottom: 1.25vh;">
        <span style="width:7.5vw; margin-right:0.5vw;">Thomas Carl Wall</span>
        <span style="width:11vw; margin-right:0.5vw;">台北科大副教授</span>
        <span style="width:20vw; margin-right:0.5vw;">華盛頓大學英美文學博士</span>
        <span style="width:31.875vw;">20世紀思想史、文學批評史、電影理論與批評</span>
      </div>
      <div class="name_block" style="width:71.875vw; display: inline-flex; margin-top: 1.25vh; margin-bottom: 1.25vh;">
        <span style="width:7.5vw; margin-right:0.5vw;">司黛蕊</span>
        <span style="width:11vw; margin-right:0.5vw;">中研院助研究員</span>
        <span style="width:20vw; margin-right:0.5vw;">美國芝加哥大學人類學博士</span>
        <span style="width:31.875vw;">次文化、民族誌田野</span>
      </div>
      <div class="name_block" style="width:71.875vw; display: inline-flex; margin-top: 1.25vh; margin-bottom: 1.25vh;">
        <span style="width:7.5vw; margin-right:0.5vw;">顏娟英</span>
        <span style="width:11vw; margin-right:0.5vw;">中研院助歷史語言研究員</span>
        <span style="width:20vw; margin-right:0.5vw;">美國哈佛大學藝術史博士</span>
        <span style="width:31.875vw;">歷史語言學研究</span>
      </div>
      <div class="name_block" style="width:71.875vw; display: inline-flex; margin-top: 1.25vh; margin-bottom: 1.25vh;">
        <span style="width:7.5vw; margin-right:0.5vw;">魏德驥</span>
        <span style="width:11vw; margin-right:0.5vw;">助理教授</span>
        <span style="width:20vw; margin-right:0.5vw;">台灣大學哲學博士</span>
        <span style="width:31.875vw;">西洋古典哲學、希臘文化史、中國古典哲學、康德哲學、美學</span>
      </div>
      <div class="name_block" style="width:71.875vw; display: inline-flex; margin-top: 1.25vh; margin-bottom: 1.25vh;">
        <span style="width:7.5vw; margin-right:0.5vw;">何東洪</span>
        <span style="width:11vw; margin-right:0.5vw;">輔仁大學心理系副教授</span>
        <span style="width:20vw; margin-right:0.5vw;">英國藍開斯特大學社會學博士</span>
        <span style="width:31.875vw;">流行音樂研究、青年文化與政策、文化經濟學、當代批判理論、社會科學質性研究</span>
      </div>
      <div class="name_block" style="width:71.875vw; display: inline-flex; margin-top: 1.25vh; margin-bottom: 1.25vh;">
        <span style="width:7.5vw; margin-right:0.5vw;">黃建宏</span>
        <span style="width:11vw; margin-right:0.5vw;">台北藝術大學美術系副教授</span>
        <span style="width:20vw; margin-right:0.5vw;">巴黎第八大學哲學所美學組博士</span>
        <span style="width:31.875vw;">哲學、電影研究、當代藝術思潮</span>
      </div>
      <div class="name_block" style="width:71.875vw; display: inline-flex; margin-top: 1.25vh; margin-bottom: 1.25vh;">
        <span style="width:7.5vw; margin-right:0.5vw;">方天賜</span>
        <span style="width:11vw; margin-right:0.5vw;">清華大學兼任助理教授</span>
        <span style="width:20vw; margin-right:0.5vw;">倫敦政治經濟學院國際關係博士</span>
        <span style="width:31.875vw;">國際關係、印度研究</span>
      </div>
      </div>
    </div>

  </div><!-- /.row -->
</div><!-- /.container -->



</?php get_template_part('includes/footer'); ?>
